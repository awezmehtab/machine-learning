...
feature_order_lamda_data = []

for feature in range(2):
    for order in range(1,4):
        if (order < 3):
            for lamda in range(0, 11, 1):
                lamda = lamda/100
                auto_data_copy = hw5.make_polynomial_feature_fun(order)(auto_data[feature].copy())
                print('Feature set:', feature+1, 'Order:', order, 'Lambda:', lamda)
                print(hw5.xval_learning_alg(auto_data_copy, auto_values, lamda, 10) * sigma)
                feature_order_lamda_data.append([feature+1, order, lamda, hw5.xval_learning_alg(auto_data_copy, auto_values, lamda, 10)[0, 0] * sigma])
        else:
            for lamda in range(0, 220, 20):
                auto_data_copy = hw5.make_polynomial_feature_fun(order)(auto_data[feature].copy())
                print('Feature set:', feature+1, 'Order:', order, 'Lambda:', lamda)
                print(hw5.xval_learning_alg(auto_data_copy, auto_values, lamda, 10) * sigma)
                feature_order_lamda_data.append([feature+1, order, lamda, hw5.xval_learning_alg(auto_data_copy, auto_values, lamda, 10)[0, 0] * sigma])