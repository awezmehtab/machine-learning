...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    trained_classifier = learner(data_train, labels_train)
    th, th0 = trained_classifier

    return float(score(data_test, labels_test, th, th0)) / data_test.shape[1]
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        ...
        performance += eval_classifier(learner, data_train, labels_train, data_test, labels_test)
    return float(performance)/it
...
def xval_learning_alg(learner, data, labels, k):
    ...
    for i in range(k):
        data_i = data_array[i]
        labels_i = labels_array[i]
        data_minus_i = data_array[:]
        data_minus_i.pop(i)
        labels_minus_i = labels_array[:]
        labels_minus_i.pop(i)
        data_train = np.concatenate(data_minus_i, axis=1)
        labels_train = np.concatenate(labels_minus_i, axis=1)
        performace += eval_classifier(learner, data_train, labels_train, data_i, labels_i)

    return float(performace)/k
...
def new_eval_learning_alg(learner, data_gen, n_train, n_test, it, params={}):
    performance = 0
    for i in range(it):
        data_train, labels_train = data_gen(n_train)
        data_test, labels_test = data_gen(n_test)
        performance += eval_classifier(learner, data_train, labels_train, data_train, labels_train)
    return float(performance)/it
    
eval = 0
for i in range(500):
    eval += new_eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 5)
print(eval/500)