...
def top_bottom_features(x):
    ...
    top_half = x[:, :m//2, :]
    bottom_half = x[:, m//2:, :]
    return np.vstack((np.mean(top_half, axis=(1,2)), np.mean(bottom_half, axis=(1,2))))

# use this function to evaluate accuracy
# acc = hw3.get_classification_accuracy(col_average_features(data), labels)
print('Accuracy for row_average_features: ', hw3.get_classification_accuracy(row_average_features(data), labels))
print('Accuracy for col_average_features: ', hw3.get_classification_accuracy(col_average_features(data), labels))
print('Accuracy for top_bottom_features: ', hw3.get_classification_accuracy(top_bottom_features(data), labels))
