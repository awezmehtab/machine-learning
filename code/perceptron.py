...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        ...
        performance += eval_classifier(
            learner, data_train, labels_train, data_test, labels_test
        )
...
def xval_learning_alg(learner, data, labels, k):
    ...
    for i in range(k):
        ...
        performace += eval_classifier(
            learner, data_train, labels_train, data_i, labels_i
        )
...
def new_eval_learning_alg(learner, data_gen, n_train, n_test, it, params={}):
    ...
    for i in range(it):
        ...
        data_test, labels_test = data_gen(n_test)
        performance += eval_classifier(
            learner, data_train, labels_train, data_train, labels_train
        )
    return float(performance) / it
...
eval = 0
for i in range(500):
    eval += new_eval_learning_alg(
        averaged_perceptron, gen_flipped_lin_separable(pflip=0.25), 20, 20, 5
    )
print(eval / 500)
