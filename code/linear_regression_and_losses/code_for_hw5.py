...
def sgd(X, y, J, dJ, w0, step_size_fn, max_iter):
    ...
    for t in range(max_iter):
        random_number = np.random.randint(n)
        random_point = np.array([X[:, random_number]]).T
        random_label = np.array([[y[0, random_number]]])
        fs.append(J(random_point, random_label, w_i.copy()))
        if t < max_iter - 1:
            w_i = w_i.copy() - step_size_fn(t) * dJ(
                random_point, random_label, w_i.copy()
            )
        ws.append(w_i)
    ws.pop()
    return (w_i, fs, ws)
...
def sgdTest():
    ...
    sgd(X, y, J, dJ, np.array([[0.0], [0.0]]), lambda x: 0.01, 1000)
...
def auto_data_and_values(auto_data, features):
    ...
    entries = {
        f: list(set([entry[f] for entry in auto_data]))
