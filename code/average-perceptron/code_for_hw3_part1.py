...
def test_linear_classifier_with_features(
    dataFun, learner, feature_fun, draw=True, refresh=True, pause=True
):
    ...
    if draw:
        ...
        def hook(params):
            (th, th0) = params
            plot_nonlin_sep(
                lambda x1, x2: int(positive(feature_fun(cv([x1, x2])), th, th0)), ax=ax
            )
...
def test_one_hot(sub):
    if (
        one_hot_internal(3, 5).tolist() == sub(3, 5).tolist()
        and one_hot_internal(4, 7).tolist() == sub(4, 7).tolist()
    ):
        print("Passed! \n")
    else:
        print("Test Failed")
...
test_with_features(xor_more, 15, draw=True, pause=False)
